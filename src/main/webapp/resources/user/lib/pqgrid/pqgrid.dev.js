		...
		if (typeof DM.getUrl == "function") {
			...
			if (typeof DM.url == "string") {
				...
				if (DM.sorting == "remote") {
					...
					if (sortingQS) {
						var sortingData = JSON.parse(sortingQS);
						sortQueryString = {
							sortColumn: sortingData[0].dataIndx,
							sortOrder: sortingData[0].dir == "down" ? "DESC" : "ASC"
						}
					}
				}
				if (PM.type == "remote") {
				...
				if (FM.type != "local") {
					...
					if (filterQS) {
						...
						filterInfo = this.getFilterData({
							CM: this.options.colModel,
							location: "remote"
						});
						filterQueryString = {
							pq_filter: filterQS,
							condition:filterInfo[0].dataIndx,
							keyword:filterInfo[0].value
						}
					}
				}
